...
import numpy as np
import pandas as pd
import NaiveAlgRanking_definition2_3_20211207 as naiveranking
    ...
    while len(S) > 0:
        ...
        if whole_cardinality < Thc:
            ...
            if smallest_valid_k > old_k:
                # this node only has one deterministic attribute, so if it is not in nodes_dict, it's impossible
                if st not in nodes_dict.keys():
