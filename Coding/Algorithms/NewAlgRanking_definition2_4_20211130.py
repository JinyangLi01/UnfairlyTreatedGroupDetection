...
def For_node_related_to_new_tuple(nodes_dict, smallest_valid_k, p, st, c, c_str, num_top_k_of_c,
    ...
    if num_top_k_of_c < lowerbound or num_top_k_of_c > upperbound:
        if st not in patterns_no_need_searching:
            patterns_no_need_searching.append(c_str)
            Remove_descendants_str(c_str, patterns_no_need_searching)
        ...
        if c_str in nodes_dict.keys():  # c_str is already in result set, I think I should pop here.
...
def GraphTraverse(ranked_data, attributes, Thc, alpha, k_min, k_max, time_limit):
    ...
    patterns_to_search_lowest_level = set()
    # stop set, but its patterns don't need any further checking. They either have too small sizes, or are in the result set.
    # allow dominance, but not between ancestors and descendants
    # TODO: this stop set should be set(), but for debugging, I use [], should change back to set()
    patterns_no_need_searching = []
    # this dict stores all patterns, indexed by num2string(p)
    nodes_dict = dict()
    time_for_CheckDominationAndAddForStopSetStr = 0
    time_for_CheckDominationAndAddForLowerbound = 0
    time_for_Remove_descendants_str = 0
    time_setup1 = 0
    ...
    while len(S) > 0:
        ...
        if whole_cardinality < Thc:
            ...
            if st not in patterns_no_need_searching:
                patterns_no_need_searching.append(st)
            time3 = time.time()
            time_for_CheckDominationAndAddForStopSetStr += time3 - time2
            continue
        time3 = time.time()
        ...
        if num_top_k < lowerbound or num_top_k > upperbound:
            if st not in patterns_no_need_searching:
                patterns_no_need_searching.append(st)
            time4 = time.time()
            time_for_CheckDominationAndAddForStopSetStr += time4 - time3
            time5 = time.time()
            CheckDominationAndAddForLowerbound(P, pattern_treated_unfairly_lowerbound)
            time6 = time.time()
            time_for_CheckDominationAndAddForLowerbound += time6 - time5
            Remove_descendants_str(st, patterns_to_search_lowest_level)
            time7 = time.time()
            time_for_Remove_descendants_str += time7 - time6
        else:
            children = GenerateChildren(P, whole_data_frame, attributes)
            if len(children) == 0:
                time9 = time.time()
                patterns_to_search_lowest_level.add(st)
                time10 = time.time()
                time_for_CheckDominationAndAddForStopSetStr += time10 - time9
    ...
    print(time_setup1, time_for_CheckDominationAndAddForStopSetStr,
          time_for_CheckDominationAndAddForLowerbound, time_Add_node_to_set)
    print("finish kmin")
    ...
    for k in range(k_min + 1, k_max):
        ...
        if st in nodes_dict.keys():
            print("after CheckCandidatesForKValues, {} in nodes_dict, smallest_valid_k = {}".format(st, nodes_dict[st].smallest_valid_k))
            # print("ancestor = {}, k of ancestor = {}".format(nodes_dict[st].smallest_ancestor,
            #                                                  nodes_dict[nodes_dict[st].smallest_ancestor].smallest_valid_k))
        # if PatternInSet(p, pattern_treated_unfairly_lowerbound):
...
def AddNewTuple(new_tuple, Thc, pattern_treated_unfairly_lowerbound,
    ...
    while len(S) > 0:
        ...
        if whole_cardinality < Thc:
            ...
            upperbound = (whole_cardinality / data_size + alpha) * k
            time2 = time.time()
            print("time for setting up before adding to result set is {}".format(time2 - time1))
            if num_top_k > upperbound:
                CheckDominationAndAddForLowerbound(P, pattern_treated_unfairly_lowerbound)
                time3 = time.time()
                if st not in patterns_no_need_searching:
                    patterns_no_need_searching.append(st)
                    Remove_descendants_str(st, patterns_no_need_searching)
                Remove_descendants_str(st, patterns_to_search_lowest_level) # TODO?????????
                time4 = time.time()
            ...
            if smallest_valid_k > old_k:
                ...
                Update_k_value(nodes_dict, smallest_valid_k, P, st, parent, parent_str, root,
                               ...
                               children_related_to_new_tuple, patterns_top_k, whole_cardinality, data_size, alpha, k,
                               patterns_size_whole, pc_whole_data, new_tuple, children)
                time2 = time.time()
                print("time for Update_k_value is {}".format(time2 - time1))
            elif st not in patterns_to_search_lowest_level:
                S = children + S
...
def CheckCandidatesForKValues_patterns_to_search_lowest_level(nodes_dict, ancestors, patterns_to_search_lowest_level,
    ...
    for st in patterns_to_search_lowest_level:  # st is a string
        ...
        # if PatternInSet([-1, -1, -1, -1, -1, 0, -1, -1, -1], pattern_treated_unfairly_lowerbound):
        #     print("in CheckCandidatesForKValues_patterns_to_search_lowest_level, in result set")
        # if st == "0|||0||0" and k == 43:
        #     print("CheckCandidatesForKValues_patterns_to_search_lowest_level, st = {}".format(st))
        num_patterns_visited += 1
        p = string2num(st)
        if p in ancestors or p in pattern_treated_unfairly_lowerbound:  # already checked
            continue
        if st in patterns_no_need_searching:
            continue
        ...
        if st not in patterns_no_need_searching:
            patterns_no_need_searching.append(child_str)
            Remove_descendants_str(child_str, patterns_no_need_searching)
        to_remove.add(st)
    for st in to_remove:
        patterns_to_search_lowest_level.remove(st)
...
def CheckCandidatesForKValues_patterns_no_need_searching(nodes_dict, ancestors, patterns_to_search_lowest_level,
    ...
    for st in patterns_no_need_searching:  # st is a string
        if st in checked_patterns:
            continue
        # if st == "|||||0||1|0" and k == 33:
        #     for re in pattern_treated_unfairly_lowerbound:
        #         if P1DominatedByP2ForStr(st, num2string(re), num_att) and A_is_ancestor_of_B(num2string(re), st):
        #             print("{} dominated by {} in result set".format(st, re))
        #     raise Exception("in CheckCandidatesForKValues_patterns_no_need_searching, st = {}".format(st))
        # print("CheckCandidatesForKValues_patterns_no_need_searching, st={}".format(st))
        # if st == "|||||0||1|0":
        #     print("        ")
        # if PatternInSet([-1, -1, -1, -1, -1, 0, -1, -1, -1], pattern_treated_unfairly_lowerbound):
        #     print("in CheckCandidatesForKValues_patterns_no_need_searching, in result set")
        parent_str = findParentForStr(st)
        ...
        child = nearest_ancestor_node
        CheckDominationAndAddForLowerbound(child.pattern, pattern_treated_unfairly_lowerbound)
        to_add.add(child_str)
        Remove_descendants_str(child_str, patterns_to_search_lowest_level)
        Remove_descendants_str(child_str, patterns_no_need_searching)
    for c in to_add:
        if c not in patterns_no_need_searching:
            patterns_no_need_searching.append(c)
