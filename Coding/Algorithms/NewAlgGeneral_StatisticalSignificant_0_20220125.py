...
return patterns, size, and fairness values
use -2 to represent non-deterministic attributes
...
def Predictive_parity(whole_data, TPdata, FPdata,
    ...
    while len(S) > 0:
        ...
        if PDominatedByM(P, pattern_with_low_fairness)[0] is False:
            ...
            fairness_values_of_patterns.append(correct_positive_prediction)
    time2 = time.time()
    ...
    return pattern_with_low_fairness, sizes_of_patterns, fairness_values_of_patterns, \
           num_patterns, time2 - time1
...
def False_positive_error_rate_balance_greater_than(whole_data, FPdata, TNdata,
    ...
    while len(S) > 0:
        ...
        #### compute t test Beta distribution
        # for dataset
        a = len(FPdata) + 1
        b = len(TNdata) + 1
        mean_d = a / (a + b)
        var_d = (a * b) / ((a + b) ** 2 * (a + b + 1))
        # for pattern
        a = fp
        b = tn
        # mean_p = a / (a + b)
        ...
        mean_p = (a + 1) / (a + b + 2)
        var_p = (a+1) * (b+1) / ((a+b+2) ** 2 * (a+b+3))
        t_value = (abs(mean_p - mean_d)) / ((var_p + var_d) ** 0.5)
        # degree of freedom
        dof = (var_p / whole_cardinality + var_d / (len(FPdata) + len(TNdata))) ** 2 / (
                    (var_p / whole_cardinality) ** 2 / (whole_cardinality - 1) +
                    (var_d / (len(FPdata) + len(TNdata))) ** 2 / ((len(FPdata) + len(TNdata)) - 1))
        p_value = stats.t.sf(t_value, dof) * 2
        from scipy.stats import t
        p_2 = 2 * (1 - t.cdf(abs(t_value), dof))
        p_3 = 2*(t.cdf(-abs(t_value), dof))
        print("p_value: {} p_2: {} p_3: {}".format(p_value, p_2, p_3))
        ### normal distribution
        ## Welch-Satterthwaite Degrees of Freedom ##

        print("\n",
              f"Welch's t-test= {t_value:.4f}", "\n",
              f"p-value = {p_value:.4f}", "\n",
              f"Welch-Satterthwaite Degrees of Freedom= {dof:.4f}")


        # p_value = stats.t.sf(abs(t_value), df=degree_of_freedom)*2
        if PDominatedByM(P, pattern_with_low_fairness)[0] is False:
            # print("pattern {} has FPR {}".format(P, FPR))
            pattern_with_low_fairness.append(P)
            sizes_of_patterns.append(whole_cardinality)
            fairness_values_of_patterns.append(FPR)
            t_values_of_patterns.append(t_value)
            p_value_of_patterns.append(p_value)
    time2 = time.time()
    # print(duration1, duration2, duration3, duration4, duration5, duration6)
    return pattern_with_low_fairness, sizes_of_patterns, fairness_values_of_patterns,\
           t_values_of_patterns, p_value_of_patterns, num_patterns, time2 - time1
...
def False_negative_error_rate_balance_greater_than(whole_data, TPdata, FNdata,
    ...
    while len(S) > 0:
        ...
        t_value = (abs(mean_p - mean_d)) / ((var_p + var_d) ** 0.5)
        # degree of freedom
        dof = (var_p / whole_cardinality + var_d / (len(FNdata) + len(TPdata))) ** 2 / (
                (var_p / whole_cardinality) ** 2 / (whole_cardinality - 1) +
                (var_d / (len(FNdata) + len(TPdata))) ** 2 / ((len(FNdata) + len(TPdata)) - 1))
        p_value = stats.t.sf(t_value, dof) * 2
        from scipy.stats import t
        p_2 = 2 * (1 - t.cdf(abs(t_value), dof))
        p_3 = 2 * (t.cdf(-abs(t_value), dof))
        ...
        print("\n",
              f"Welch's t-test= {t_value:.4f}", "\n",
              f"p-value = {p_value:.9f}", "\n",
              f"Welch-Satterthwaite Degrees of Freedom= {dof:.4f}")

        if PDominatedByM(P, pattern_with_low_fairness)[0] is False:
            pattern_with_low_fairness.append(P)
            sizes_of_patterns.append(whole_cardinality)
            fairness_values_of_patterns.append(FNR)
            t_values_of_patterns.append(t_value)
            p_value_of_patterns.append(p_value)
    time2 = time.time()
    # print(duration1, duration2, duration3, duration4, duration5, duration6)
    return pattern_with_low_fairness, sizes_of_patterns, fairness_values_of_patterns,\
           t_values_of_patterns, p_value_of_patterns, \
           num_patterns, time2 - time1
...
def Equalized_odds(whole_data, TPdata, TNdata, FPdata, FNdata,
    ...
    while len(S) > 0:
        ...
        if PDominatedByM(P, pattern_with_low_fairness)[0] is False:
            pattern_with_low_fairness.append(P)
            sizes_of_patterns.append(whole_cardinality)
            fairness_values_of_patterns.append((FNR, FPR))
...
def Conditional_use_accuracy_equality(whole_data, TPdata, TNdata, FPdata, FNdata,
    ...
    while len(S) > 0:
        ...
        if PDominatedByM(P, pattern_with_low_fairness)[0] is False:
            pattern_with_low_fairness.append(P)
            sizes_of_patterns.append(whole_cardinality)
            fairness_values_of_patterns.append((positive_prob, negative_prob))
    time2 = time.time()
    # print(duration1, duration2, duration3, duration4, duration5, duration6)
    return pattern_with_low_fairness, sizes_of_patterns, fairness_values_of_patterns,\
           num_patterns, time2 - time1
...
def Treatment_equality(whole_data, TPdata, TNdata, FPdata, FNdata,
    ...
    while len(S) > 0:
        ...
        if PDominatedByM(P, pattern_with_low_fairness)[0] is False:
            pattern_with_low_fairness.append(P)
            sizes_of_patterns.append(whole_cardinality)
            fairness_values_of_patterns.append(ratio)
    time2 = time.time()
    # print(duration1, duration2, duration3, duration4, duration5, duration6)
    return pattern_with_low_fairness, sizes_of_patterns, fairness_values_of_patterns,\
           num_patterns, time2 - time1
