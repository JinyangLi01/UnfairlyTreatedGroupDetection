...
selected_attributes = all_attributes[:21] + ['absences_C']
...
for bin in binnings:
    ...
    original_data_file = r"../../../InputData/StudentDataset/DifferentBinning/student-mat_cat_ranked_"
    original_data_file += str(bin) + 'absences.csv'
...
output_path = r'../../../OutputData/Binning/StudentData/binning_prop_student_22att.txt'
output_file = open(output_path, "w")
num_lines = len(execution_time1)

output_file.write("execution time\n")
for n in range(len(binnings)):
    output_file.write('{} {} {}\n'.format(binnings[n], execution_time1[n], execution_time2[n]))


output_file.write("\n\nnumber of patterns visited\n")
for n in range(len(binnings)):
    output_file.write('{} {} {}\n'.format(binnings[n], num_patterns_visited1[n], num_patterns_visited2[n]))


output_file.write("\n\nnumber of patterns found, lowerbound\n")
for n in range(len(binnings)):
    output_file.write('{} {} \n {}\n'.format(binnings[n], num_patterns_found_lowerbound[n], patterns_found_lowerbound[n]))
...
plt.plot(binnings, execution_time1, line_style[0], color=color[0], label=label[0], linewidth=line_width,
          markersize=marker_size)
plt.plot(binnings, execution_time2, line_style[1], color=color[1], label=label[1], linewidth=line_width,
             markersize=marker_size)
...
plt.grid(True)
fig.tight_layout()
plt.savefig("../../../OutputData/Binning/StudentData/binning_time_prop_student_22att.png",
            bbox_inches='tight')
plt.show()
plt.close()
...
fig, ax = plt.subplots(1, 1, figsize=f_size)
plt.plot(binnings, num_patterns_visited1, line_style[0], color=color[0], label=label[0], linewidth=line_width,
          markersize=marker_size)
plt.plot(binnings, num_patterns_visited2, line_style[1], color=color[1], label=label[1], linewidth=line_width,
             markersize=marker_size)
plt.xlabel('Number of bins')
plt.xticks(binnings)
...
plt.grid(True)
fig.tight_layout()
plt.savefig("../../../OutputData/Binning/StudentData/binning_calculations_prop_student_22att.png",
            bbox_inches='tight')
plt.show()
plt.close()
